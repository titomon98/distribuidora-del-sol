...
const ComprasListado = () => {
	...
	const ver = async (row) => {
		setCargando(true);
		setDetalle({ numeroCompra: row.numeroCompra, proveedor: row.proveedor, total: row.total, items: [] });
		try {
			const { data } = await axiosInstance.get(`/compras/${row.id}`);
			setDetalle(data);
	...
	return (
		<>
			<ListView
				...
				exportable exportName="compras" totalField="total"
				columns={[
				...
				actions={(row) => (
					<button className="btn btn-sm btn-info light" title="Ver detalle" onClick={() => ver(row)}>
			...
			<Modal show={!!detalle} onHide={() => setDetalle(null)} centered>
				<div className="modal-header">
					<h5 className="modal-title">Compra {detalle?.numeroCompra || detalle?.numero_compra || ""}</h5>
					<button type="button" className="btn-close" onClick={() => setDetalle(null)}></button>
				</div>
				<div className="modal-body">
					<p className="mb-2"><strong>Proveedor:</strong> {detalle?.proveedor || "-"}</p>
